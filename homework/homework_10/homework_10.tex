...
\begin{soln}
    ...
    \begin{enumerate}
        ...
        \noindent Observe that the inverse relation $f'$ may not be a function
        because $B'$ is not always $B$. However, if we restrict it's domain
        to $B'$ so that $f'_{\text{res}} : B' \rightarrow A$, then it is
        both a function and bijective. 

        \noindent Similarly, if we go back and restrict $f$'s codomain such that
        $f_{\text{res}} : A \rightarrow B'$ then it is also bijective.

        \noindent Thus, $f^{-1}_{\text{res}}(f_\text{res}(x)) = i_A$.

        \noindent This ensures that for every $x \in X$, $x = f^{-1}(f(x))$. 
